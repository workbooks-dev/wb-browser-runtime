...
//
// Signed-URL diagnostics: some SaaS exports never fire a `download` event —
// the page fetches same-origin JSON carrying a pre-signed object-store URL and
// navigates to it, leaving nothing for the listener above to catch. We install
// the signed-URL fetch/XHR hook (lib/signed-url-capture.js) context-wide and,
// at clean slice end, emit `slice.download_skipped { reason:
// "signed_url_not_captured" }` for any recognized signed-host URL that wasn't
// claimed by the explicit `download:` verb. This is diagnostics only: passive
// mode never fetches the URL itself — server-side capture (with SSRF guards)
// is the `download:` verb's job.
...
export function installDownloadCapture(context, getCurrentVerb) {
  ...
  // Signed-URL diagnostics. Some SaaS exports never fire a `download` event:
  // the page fetches same-origin JSON carrying a pre-signed object-store URL,
  // then navigates to it and the bytes either render inline or are CORS-blocked
  // — nothing lands and the passive listener above sees nothing. Install the
  // signed-URL fetch/XHR hook context-wide (re-runs on every document, incl.
  // cross-origin) so those responses are observed even when no `download:`
  // verb is racing. We do NOT auto-download here — fetching an arbitrary URL
  // the page mentioned belongs to the explicit `download:` verb with its SSRF
  // guards. We only surface a `slice.download_skipped` diagnostic so the
  // operator learns an export went uncaptured and how to grab it.
  try {
    context.addInitScript?.(SIGNED_PAGE_HOOK)?.catch?.(() => {});
  } catch {}
  // Cover pages already open at install time (restored sessions land on the
  // app page before this runs; addInitScript only affects future documents).
  try {
    for (const p of context.pages?.() ?? []) {
      p.evaluate?.(SIGNED_PAGE_HOOK)?.catch?.(() => {});
    }
  } catch {}

  return {
    drainSignedDiagnostics: () =>
      drainSignedDiagnostics({ context, getCurrentVerb, seen }),
  };
}

// Process-wide dedup of reported signed URLs (by redacted origin+path) so an
// export referenced across several slices — or several sessions in the same
// sidecar process — is announced once, not on every drain.
const seen = new Set();

// Read-and-clear the page-side signed-URL candidate buffer across every open
// page and emit a `slice.download_skipped { reason: "signed_url_not_captured" }`
// for each recognized signed-host URL that wasn't already consumed by an
// explicit `download:` verb (the verb's own read-and-clear drains the ones it
// captured, so only the genuinely-missed exports remain here). Best-effort and
// never throws — diagnostics must not be able to fail a clean slice.
async function drainSignedDiagnostics({ context, getCurrentVerb, seen }) {
  ...
  for (const page of pages) {
    let candidates = [];
    try {
      candidates = (await page.evaluate(SIGNED_POLL_SCRIPT)) || [];
    } catch {
      // Page navigating/closed — skip; another page or the next slice covers it.
      continue;
    }
    let pageUrl = null;
    try {
    ...
    for (const cand of candidates) {
      for (const u of cand?.urls ?? []) {
        ...
        // Auto policy: only recognized object-store / CDN hosts. Same gate the
        // verb uses in auto mode — keeps us from flagging ordinary app links.
        if (!isSignedHost(host)) continue;
        const redacted = redactSignedUrl(u.url);
        if (seen.has(redacted)) continue;
        seen.add(redacted);
        send({
          type: "slice.download_skipped",
          reason: "signed_url_not_captured",
          capture: "signed_url",
          signed_url: redacted,
          api_url: cand.api_url ?? null,
          field: u.field ?? null,
          host,
          page_url: pageUrl,
          verb_index: verb.index ?? null,
          verb_name: verb.name ?? null,
          hint: 'A same-origin export response referenced a signed file URL that was not captured. Use the `download:` verb with `signed_url` to fetch it server-side.',
          ts: Date.now(),
        });
      }
    }
